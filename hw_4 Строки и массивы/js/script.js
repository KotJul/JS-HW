...
for(let i = 0; i < arr.length; i++){
  summ += arr[i];// **сумму всех елементов 
  arithMean = (arithMean + arr[i]) / arr.length;// **среднее арифметическоe всех элементов


  if(max < arr[i]){// **максимальное число и минимальное число 
    max = arr[i];
  }
    if(min > arr[i]){
     min = arr[i];
  }if(i % 2 === 0){
    summCh = summCh + arr[i];// **сумму всех элементов на четных позициях
  }
}
console.log(summ,max,min,arithMean,summCh);
...
for ( let i=0 ; i < words.length; i++){
  let word = words[i];
  word = word[0].toUpperCase() + word.slice(1);/* в консоле выдает ошибку*/
  words[i]=word;
  let p = words[i];
if (p.startsWith('р')){
    first += p;
  }
else if (str.includes('е')){
  str = str.replaceAll('е', 'и');
} 
...
console.log('Все буквы с "Р" : ', first);
console.log('Words:', words);
console.log(str);}